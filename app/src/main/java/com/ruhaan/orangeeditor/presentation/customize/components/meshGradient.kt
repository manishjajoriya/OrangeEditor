  ...
  return drawBehind {
    drawIntoCanvas { canvas ->
      ...
      if (showPoints) {
        ...
        flattenedPaint.color = Color.Red.copy(alpha = .9f)
...
class PointData(
  ...
  init {
    offsets =
        buildList { repeat((xLength - 0) * (yLength - 0)) { add(Offset(0f, 0f)) } }.toMutableList()
  ...
  private fun generateInterpolatedOffsets() {
    ...
    for (y in 0..<points.lastIndex) {
      for (x in 0..<this.xLength) {
        ...
        for (i in (1..<stepsY)) {
          val point3 =
              measure.getPosition(i / stepsY.toFloat() * measure.length).let { Offset(it.x, it.y) }
      ...
      Path().apply {
        ...
        when (position) {
          ...
          2 -> cubicTo(point1.x + delta, point1.y, point2.x, point2.y, point2.x, point2.y)
              ...
              cubicTo(point1.x + delta, point1.y, point2.x - delta, point2.y, point2.x, point2.y)
        }
      ...
      Path().apply {
        ...
        when (position) {
          ...
          2 -> cubicTo(point1.x, point1.y + delta, point2.x, point2.y, point2.x, point2.y)
              ...
              cubicTo(point1.x, point1.y + delta, point2.x, point2.y - delta, point2.x, point2.y)
        }
